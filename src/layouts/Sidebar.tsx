...
import {
  LayoutDashboard, Truck, Map, Users, Cpu, AlertTriangle,
  BarChart3, ScrollText, Settings, ChevronLeft, ChevronRight,
  Mountain, Shield
} from 'lucide-react'
...
export function Sidebar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className={cn('flex items-center border-b border-slate-700/60 px-4 py-4', sidebarCollapsed ? 'justify-center' : 'gap-3')}>
        ...
        {!sidebarCollapsed && (
          <div className="overflow-hidden">
            <p className="text-sm font-bold text-slate-100 leading-none">SMART MINE</p>
            <p className="text-[10px] font-medium text-primary-400 tracking-widest mt-0.5">RDC</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3 px-2 space-y-0.5">
        {navItems.map(({ path, label, icon: Icon }) => {
          ...
          const link = (
            ...
            >
              <Icon className={cn('h-4 w-4 shrink-0', isActive ? 'text-primary-400' : '')} />
              {!sidebarCollapsed && <span className="truncate">{label}</span>}
            </NavLink>
          )
          return sidebarCollapsed
            ? <Tooltip key={path} content={label} side="right">{link}</Tooltip>
      ...
      {!sidebarCollapsed && (
        <div className="border-t border-slate-700/60 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] text-slate-500">Système sécurisé · v2.4.1</p>
          </div>
        </div>
      )}
